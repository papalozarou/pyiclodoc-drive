    ...
    def test_load_config_builds_expected_paths(self) -> None:
        with tempfile.TemporaryDirectory() as TMPDIR:
            ...
            with patch.dict(os.environ, BASE_ENV, clear=True):
                ...
                self.assertEqual(CONFIG.manifest_path, Path(BASE_ENV["CONFIG_DIR"]) / "manifest.json")
                self.assertEqual(CONFIG.auth_state_path, Path(BASE_ENV["CONFIG_DIR"]) / "auth_state.json")
                self.assertEqual(CONFIG.heartbeat_path, Path(BASE_ENV["LOGS_DIR"]) / "heartbeat.txt")
